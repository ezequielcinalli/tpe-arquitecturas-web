...
public interface StudentCareerRepository extends JpaRepository<StudentCareer, Integer> {
    ...
    @Query("SELECT new com.example.tp3.Dtos.CareerWithInscriptionsDto(c.name,COUNT(sc.student)) FROM StudentCareer sc JOIN sc.career c GROUP BY sc.career.id ORDER BY count(sc.student.id)")
